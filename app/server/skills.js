...
// ── compare_mortgage_offer ───────────────────────────────────
function compareMortgageOffer({
  ...
  const comparisons = [
    {
      name:          'Rata mensile',
      ideal_label:   `${fmtEur(idealRata)}/mese (max sostenibile)`,
      offered_label: `${fmtEur(actualPayment)}/mese`,
    ...
    },
    {
      ...
      ideal_label:   idealAmount > 0 ? `${fmtEur(idealAmount)} (max calcolato per te)` : 'Non calcolabile',
      offered_label: `${fmtEur(amount)} (richiesto)`,
      ...
      note: idealAmount <= 0 ? '—'
        : amount <= idealAmount
          ...
          : `${fmtEur(amount - idealAmount)} oltre l'importo ideale — verifica la sostenibilità`,
  ...
  return {
    ...
    overall_label: {
      ok:      'Preventivo in linea col tuo profilo ideale',
      warning: 'Preventivo accettabile con riserve',
      danger:  'Preventivo distante dal tuo profilo ideale',
    }[overall],
...
const SKILLS = {
  ...
  compare_mortgage_offer: compareMortgageOffer,
